...
def hyperopt_search(hyper_choice,inputAfterSelect,targetName,classifierList,maxIter,hyper_folder):
    parameters = {
                  'rf':{
    ...
    if hyper_choice == 2:
        ...
        if 'xgb' in classifierList:
            ...
            optimalModel = xgb.XGBClassifier(silent=True,nthread=n_jobs,seed=333,objective='binary:logistic',learning_rate=best['learning_rate'],subsample=best['subsample'],max_depth=parameters['xgb']['max_depth'][best['max_depth']],n_estimators=parameters['xgb']['n_estimators'][best['n_estimators']],colsample_bytree=best['colsample_bytree'],min_child_weight=parameters['xgb']['min_child_weight'][best['min_child_weight']])
