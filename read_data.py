...
def read_data(read_choice,read_folder):
    ...
    if read_choice == 0:
        ...
        CLR_1 = pd.read_csv('%s/ACH_CLR_1.csv'%read_folder)
        CLR_2 = pd.read_csv('%s/ACH_CLR_2.csv'%read_folder)
        IDA = pd.read_csv('%s/ACH_IDA.csv'%read_folder)
        RL = pd.read_csv('%s/ACH_RL.csv'%read_folder)
        TU = pd.read_csv('%s/ACH_TU.csv'%read_folder)
        EXP = pd.read_csv('%s/ACH_EXP.csv'%read_folder)
...
#        target_fpd_df = pd.concat([inputData['fpd'],inputData['fpd_x'],inputData['fpd_y']],axis=1)
        target_fpd_df = pd.concat([inputData['ACH_x'],inputData['ACH_y']],axis=1)
        target_fpd = target_fpd_df.max(axis=1)
#        inputData.drop(['fpd','fpd_x','fpd_y'],axis=1,inplace=True)
        inputData.drop(['ACH_x','ACH_y'],axis=1,inplace=True)
        inputData['ACH']=target_fpd
